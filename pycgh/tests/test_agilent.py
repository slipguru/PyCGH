...
def test_access_features():
    ...
    assert_equal(1856, len(aCGH))
    assert_equal(1856, len(aCGH['id']))
    assert_equal(1746, len(aCGH.F['id']))
...
def test_qc_cleaning():
    ...
    assert_equal(1856, len(aCGH))
    assert_equal(110, aCGH['mask'].sum())

    aCGH = agilent(SAMPLE_PATH, qc_masking=True)
    assert_equal(1856, len(aCGH))
    assert_equal(110 + 1, aCGH['mask'].sum())
...
def test_qc_missings():
    ...
    assert_equal(45220, len(aCGH))
    assert_equal(110 + 1 + (45220 - 1856), aCGH['mask'].sum())
...
def test_content():
    ...
    # Find present data
    present = np.logical_and(aCGH['row'] == 82, aCGH['col'] == 52)
    assert_equal(82, aCGH['row'][present])
    assert_equal(52, aCGH['col'][present])
    assert_equal('A_14_P201743', aCGH['id'][present])
    assert_equal(13, aCGH['chromosome'][present])
    assert_equal(95517847, aCGH['start_base'][present])
    assert_equal(95517906, aCGH['end_base'][present])
...
1935	chr4	177036969	177037029	A_14_P137200
1191	chr8	79514725	79514785	A_14_P108435
"""

def test_release_mapping():
    aCGH = agilent(SAMPLE_PATH)
    index = (aCGH['id'] == 'A_14_P103951')
    assert_equal(22, aCGH['chromosome'][index])
    assert_equal(28966424, aCGH['start_base'][index])
