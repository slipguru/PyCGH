FLLat.PVE <- function(Y,J.seq=seq(1,min(15,floor(ncol(Y)/2)),by=2),
                      B=c("pc","rand"),T=c("std"),lams=c("same","diff"),thresh=10^(-4),
  ...
  T <- match.arg(T)
  lams <- match.arg(lams)
  CheckPars(Y=Y,B=B,T=T,thresh=thresh,maxiter=maxiter,maxiter.B=maxiter.B,
            maxiter.T=maxiter.T)
  ...
  if (lams=="same") {
    bic <- FLLat.BIC(Y,B=B,T=T,thresh=thresh,maxiter=maxiter,
                     maxiter.B=maxiter.B,maxiter.T=maxiter.T)
  }

  for (i in 1:length(J.seq)) {
    if (lams=="diff") {
      est <- FLLat.BIC(Y,J.seq[i],B,T,thresh,maxiter,maxiter.B,
                       maxiter.T)$opt.FLLat
    } else {
      est <- FLLat(Y,J.seq[i],B,T,bic$lam1,bic$lam2,thresh,maxiter,
                   maxiter.B,maxiter.T)
    }
